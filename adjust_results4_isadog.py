...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    """           
    # create dic for dog names
    dognames_dic = dict()
    
    # read dog name file as line
    with open(dogfile, "r") as infile:
        line = infile.readline()
        
        # adding dognames to dognames_dic
        while line != "":
            # striping newline from line
            line = line.rstrip()
            # adds dogname(line) to dogsnames_dic if it doesn't already exist in dogsnames_dic
            if line not in dognames_dic:
                dognames_dic[line] = line
            # read the next line if not empty
            else:
                line = infile.readline()
